class Rank < ActiveRecord::Base
  ...
  def self.rank_me(resource,args,user_id)
    ...
    if vote.present?
      vote.destroy and resource.rank -= 1 if vote.vote == 1 && (( args[:vote_down] && User.can_vote?(user_id)) || args[:vote_up])
      vote.destroy and resource.rank += 1 if vote.vote == 0  && User.can_vote?(user_id)
    else
